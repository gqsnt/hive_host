...
impl UserPageType {
    pub fn href(&self) -> String {
        match self {
            UserPageType::Dashboard => "/user".to_string(),
            UserPageType::Projects => "/user/projects".to_string(),
            UserPageType::Settings => "/user/settings".to_string(),
        }
    }

    pub fn label(&self) -> String {
        match self {
            UserPageType::Dashboard => "Dashboard".to_string(),
            UserPageType::Projects => "Projects".to_string(),
            UserPageType::Settings => "Settings".to_string(),
        }
    }

    pub fn svg(&self) -> String {
        match self {
            UserPageType::Dashboard => "m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25".to_string(),
            UserPageType::Projects => "M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z".to_string(),
            UserPageType::Settings =>     "M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z".to_string(),
        }
    }
}

impl From<&str> for UserPageType {
    fn from(page: &str) -> Self {
        let paths = page.trim_start_matches('/').split('/').collect::<Vec<_>>();
        match paths.get(1) {
            None => UserPageType::default(),
            Some(&path) => match path {
                "dashboard" => UserPageType::Dashboard,
                "projects" => UserPageType::Projects,
                "settings" => UserPageType::Settings,
                _ => UserPageType::default(),
            },
        }
    }
}

pub type UserSignal = Signal<Option<User>>;
        ...
        <div class="dark h-full">
            <Transition>
                {move || Suspend::new(async move {
                    ...
                    let menu_icon_path = "M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5".to_string();
                    ...
                    match user {
                        Ok(user) => {
                            global_store.update(|inner| inner.user = Some((user.get_slug(), user)));
                        }
                        Err(_) => {
                            global_store.update(|inner| inner.user = None);
                        }
                    }
                    match csrf {
                        Ok(csrf) => {
                                    ...
                                    <div class="mt-auto -mx-2 space-y-1">
                                        <li>
                                            <A
                                                ...
                                                class:bg-gray-800=move || current_page.get() == UserPageType::Settings
                                                class:text-white=move || current_page.get() == UserPageType::Settings
                                                class:text-gray-400=move || current_page.get() != UserPageType::Settings
                                                class:hover:text-white=move || current_page.get() != UserPageType::Settings
                                                class:hover:bg-gray-800=move || current_page.get() != UserPageType::Settings
                                            >
                                                <svg
                                                    class="size-6 shrink-0"
                                                    class:text-white=move || current_page.get() == UserPageType::Settings
                                                    class:text-gray-400=move || current_page.get() != UserPageType::Settings
                                                    class:group-hover:text-white=move || current_page.get() != UserPageType::Settings
                                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"
                                                >
                                                  <path stroke-linecap="round" stroke-linejoin="round" d=UserPageType::Settings.svg()></path>
                                                </svg>
...
#[component]
fn NavItem(
    #[prop(into)] page: UserPageType,
    #[prop(into)] current_page: ReadSignal<UserPageType>,
) -> impl IntoView {
    view! {
        <li>
            <A
